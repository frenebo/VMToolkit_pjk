...
namespace VMTutorial
{
  ...
  void export_Mesh(py::module& m)
  {
    py::class_<Mesh>(m, "Tissue")
      ...
      .def("get_vertex", &Mesh::get_vertex_reference , py::return_value_policy::reference)
      // .def("get_junction", &Mesh::get_halfedge, py::return_value_policy::reference)
      .def("get_cell", &Mesh::get_face_reference, py::return_value_policy::reference)
      ...
      // .def("get_cell_centre", [](Mesh& m, int i) -> Vec { return m.get_face_centre(*(m.get_face(i))); })
      .def("get_cell_centroid", [](Mesh& m, int i) -> Vec { return m.get_face_centroid(*(m.get_face(i))); })
      // .def("get_centre", &Mesh::get_centre)
      .def("cell_area", [](Mesh &m, int i) -> double { return m.area(*(m.get_face(i))); })
      .def("cell_perim", [](Mesh &m, int i) -> double { return m.perim(*(m.get_face(i))); })
      .def("get_vertex_positions", &Mesh::get_vertex_positions)
      .def("get_face_member_vertex_ids", &Mesh::get_face_member_vertex_ids)
      .def("set_logflag", &Mesh::set_logflag, py::arg("logname"), py::arg("new_val"))
      ;
  }
